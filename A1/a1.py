...
def partition_girvan_newman(graph, max_depth):
    ...
    """
    ###TODO
    partitions = [p for p in nx.connected_component_subgraphs(graph)]
    while len(partitions) == 1:
        unwanted_edges = sorted(approximate_betweenness(graph, max_depth).items(), key=lambda x:x[1], reverse = True)[0][0]
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        list2 = partitions[1]
#        new_graph = graph
        score.append((depth,norm_cut(list1, list2, new_graph)))
    return score
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    for x in range(0,n):
        new_graph.remove_edge(neighbors[x],test_node)
    return new_graph
    pass
